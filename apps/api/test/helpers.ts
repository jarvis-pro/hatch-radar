...
export async function truncateAll(db: AppDatabase): Promise<void> {
  await db.$executeRawUnsafe(
    'TRUNCATE task_stages, tasks, runs, processes, blueprints, request_queue, request_lanes, translations, sync_ops, triage, insights, comments, posts, provider_api_keys, model_providers, sources, source_connectors, app_settings, users RESTART IDENTITY CASCADE',
  );
}
