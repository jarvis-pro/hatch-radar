...
import {
  ...
  toConnectorDTO,
  type ConnectorInput,
  type CrawlerConfigService,
  type SourceConnectorsRepository,
  type SourcePlatform,
...
/**
 * /api/sources/* —— 采集来源（爬虫计划）CRUD + 概览（需 settings:manage）。
...
export class SourcesController {
  ...
  @Inject(TOK.sourceConnectors)
  connectors!: SourceConnectorsRepository;
  ...
  @Post('/')
  @HttpCode(201)
  async create(@ValidBody(createSourceSchema) dto: z.infer<typeof createSourceSchema>) {
  ...
  @Put('/:id')
  async update(@IntParam('id') id: number, @ValidBody(updateSourceSchema) dto: z.infer<typeof updateSourceSchema>) {
    const existing = await this.sources.getSource(id);
    if (!existing) throw new httpError.NotFoundError('来源不存在');
  ...
  async remove(@IntParam('id') id: number) {
    if (!(await this.sources.deleteSource(id))) throw new httpError.NotFoundError('来源不存在');
  ...
  private async assertRedditEnable(platform: SourcePlatform, enabling: boolean): Promise<void> {
    if (platform === 'reddit' && enabling && !(await this.connectors.hasUsableConnector('reddit'))) {
...
/**
 * /api/source-connectors/* —— 采集连接器（需鉴权平台的凭据）CRUD + 连通性测试（需 settings:manage）。
...
export class SourceConnectorsController {
  ...
  @Inject(TOK.crawlerConfig)
  crawlerConfig!: CrawlerConfigService;

  /** POST /api/source-connectors —— 新建连接器（凭据加密入库），201 { id } */
  @Post('/')
  @HttpCode(201)
  async create(@ValidBody(createConnectorSchema) dto: z.infer<typeof createConnectorSchema>) {
  ...
  @Put('/:id')
  async update(@IntParam('id') id: number, @ValidBody(updateConnectorSchema) dto: z.infer<typeof updateConnectorSchema>) {
    if (dto.secret && !isSecretConfigured()) {
      throw new httpError.BadRequestError('未配置 SETTINGS_SECRET，无法加密新凭据');
    }
    if (!(await this.connectors.getConnector(id))) throw new httpError.NotFoundError('连接器不存在');
  ...
  @Del('/:id')
  async remove(@IntParam('id') id: number) {
    if (!(await this.connectors.deleteConnector(id))) throw new httpError.NotFoundError('连接器不存在');
    logger.info(`[采集连接器] 删除 #${id}`);
    return { ok: true };
  }

  /** POST /api/source-connectors/:id/test —— 连通性测试并记录结果，始终 { ok, error? }（201） */
  @Post('/:id/test')
  @HttpCode(201)
