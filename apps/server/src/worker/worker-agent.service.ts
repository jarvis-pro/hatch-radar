...
/**
 * Worker 侧 WebSocket 客户端：连接 GatewayService，接收分发的任务并执行。
 *
 * 启动时发送 register → 等待 dispatch → 执行 → 回报 job_result。
 * 断连后指数退避重连，重建后注册表自动恢复。
 */
@Injectable()
export class WorkerAgentService implements OnApplicationBootstrap, OnApplicationShutdown {
