...
export class AdminService {
  ...
  async createUser(actor: AuthedUser, dto: CreateUserDto): Promise<{ id: string }> {
    ...
    if (!email || !name) throw new BadRequestException('邮箱与姓名必填');
    if (dto.password.length < 8) throw new BadRequestException('初始密码至少 8 位');
    if (dto.role === 'super_admin' && actor.role !== 'super_admin') {
      throw new ForbiddenException('只有超级管理员能创建超级管理员');
    }
    if (await this.users.findByEmail(email)) throw new ConflictException('该邮箱已存在');
  ...
  async editUser(actor: AuthedUser, userId: string, dto: EditUserDto): Promise<void> {
    const name = dto.name.trim();
    if (!name) throw new BadRequestException('参数不完整');
    const target = await this.users.findById(userId);
    if (!target) throw new NotFoundException('账户不存在');
    if (target.role === 'super_admin' && actor.role !== 'super_admin') {
      throw new ForbiddenException('只有超级管理员能管理超级管理员');
    }
    if (dto.role === 'super_admin' && actor.role !== 'super_admin') {
      throw new ForbiddenException('只有超级管理员能授予超管角色');
    }
    if (
  ...
  async resetPassword(actor: AuthedUser, userId: string): Promise<{ tempPassword: string }> {
    ...
    if (!target) throw new NotFoundException('账户不存在');
    if (target.role === 'super_admin' && actor.role !== 'super_admin') {
      throw new ForbiddenException('只有超级管理员能重置超级管理员密码');
    }
  ...
  async setStatus(actor: AuthedUser, userId: string, status: 'active' | 'disabled'): Promise<void> {
    if (userId === actor.id) throw new BadRequestException('不能停用 / 启用自己');
    const target = await this.users.findById(userId);
    if (!target) throw new NotFoundException('账户不存在');
    if (target.role === 'super_admin' && actor.role !== 'super_admin') {
      throw new ForbiddenException('只有超级管理员能管理超级管理员');
    ...
    if (status === 'disabled' && (await this.isLastActiveSuper(userId))) {
      throw new BadRequestException('不能停用最后一个超级管理员');
    }
  ...
  async deleteUser(actor: AuthedUser, userId: string): Promise<void> {
    if (userId === actor.id) throw new BadRequestException('不能删除自己');
    const target = await this.users.findById(userId);
    if (!target) throw new NotFoundException('账户不存在');
    if (target.role === 'super_admin' && actor.role !== 'super_admin') {
      throw new ForbiddenException('只有超级管理员能删除超级管理员');
    }
  ...
  async createEnrollment(
    ...
    if (!name) throw new BadRequestException('请填写设备名');
    const ttl = ALLOWED_TTL_DAYS.includes(ttlDays) ? ttlDays : 30;
    const target = await this.users.findById(userId);
    if (!target) throw new NotFoundException('账户不存在');
    if (target.role === 'super_admin' && actor.role !== 'super_admin') {
      throw new ForbiddenException('只有超级管理员能为超级管理员赋予设备');
    }
  ...
  async revokeDevice(actor: AuthedUser, credentialId: string): Promise<void> {
    ...
    if (!cred) throw new NotFoundException('设备不存在');
    if (cred.ownerRole === 'super_admin' && actor.role !== 'super_admin') {
      throw new ForbiddenException('只有超级管理员能管理超级管理员的设备');
    }
