 ...
 *
 * 多 Key 故障转移：处理器不绑定具体 Key——每次 analyze 时按 priority 实时挑「可用」Key，
 * 逐把尝试（限流即冷却、鉴权失败即失效，均切下一把），故 Key 健康变化对下条任务即时生效。
 */
@Injectable()
export class AnalysisConfigService {
   ...
   * - 配置不存在 / 已停用 时返回 null（由调用方判失败，不崩进程）
   * - 处理器只捕获 provider 标量；Key 在每次 analyze 时实时选取，故 Key 增删/启停即时生效
   ...
   * - 限流(429)：把该 Key 冷却 {@link COOLDOWN_SECONDS}s 后切下一把
   * - 鉴权失败/额度耗尽(401/403)：把该 Key 标失效（需人工复位）后切下一把
   * - 其它错误（网络/超时/5xx，provider 已内部退避）：不动 Key、直接冒泡，由 worker 计失败
   * - 任一把成功即返回；成功时若该把原为非 active（冷却后恢复）则复位为 active（自愈）
  ...
  /**
   * 热重载分析配置：递增库中 config 版本并清空本进程处理器缓存，使模型标量（provider/model/
   * base_url/启用项）的变更对下一条任务即时生效。在任意设置写操作后调用（无需重启进程）。
   * 递增版本号是为了让独立 worker 进程（持各自缓存实例）也能在下个任务感知变更。
   * 注：Key 的增删/启停/健康态不走缓存——每次 analyze 实时查库，本就即时生效。
   */
