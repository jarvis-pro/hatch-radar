...
export async function truncateAll(db: AppDatabase): Promise<void> {
  await db.$executeRawUnsafe(
    'TRUNCATE analysis_jobs, sync_ops, triage, insights, comments, posts, provider_api_keys, model_providers, sources, source_connectors, app_settings RESTART IDENTITY CASCADE',
  );
}
