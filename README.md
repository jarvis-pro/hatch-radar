...
| Web 控制台    | Vite + React Router SPA（纯 CSR，经 `/api` 调后端；build 产物单独部署托管，`/api` 反代到后端）                                                                         |
...
后端的两个必填项放在 `apps/api/.env`，进程启动时自动加载（`prisma migrate` 也用它）：
...
> 其余项均有默认值、按需取消注释即可：超管种子 `SUPER_ADMIN_EMAIL` / `SUPER_ADMIN_PASSWORD`、调优 `LOG_LEVEL` / `HTTP_PORT` / 内嵌执行器并发 `WORKER_CONCURRENCY` / `DATABASE_POOL_MAX`，详见 `apps/api/.env.example`。
...
```bash
# 后端 api（单进程：HTTP /api + 爬取 + 调度 + 内嵌任务执行）
...
- dev：`pnpm dev:web`（Vite 47080）把 `/api` 代理到后端（47878）保持同源；prod：`pnpm build:web` 出 `dist/`，单独部署（任意静态托管 / CDN），由反向代理把 `/api` 转发到后端
...
**② 后端容器化（`--profile full`）**：db + api 进容器（仅后端）；后端 API 在 `http://localhost:47878/api`，web 单独部署、不在本 compose。

```bash
docker compose --profile full up -d --build   # 起 / 重建全栈（首次或改代码后加 --build）
```

> 全栈下跑 AI 须用 API Key 模式 provider（设置页配 + `apps/api/.env` 填 `SETTINGS_SECRET`）；claude_cli 订阅模式依赖宿主机登录态、仅适合轻量开发。后端镜像见根 `Dockerfile`（跑 TS 源）。
...
端口 `HTTP_PORT`（默认 47878），发 `/api`（web 单独部署、经反向代理同域调用）。鉴权恒开、fail-closed（网络位置不参与鉴权）：web 面向端点走会话 cookie + 能力闸（`SessionAuthGuard`），导出 / 同步走「设备签名 **或** 用户会话」双通道（`DeviceOrSessionGuard`），设备激活由一次性激活码自鉴权，`/api/health` 公开。
...
> Reddit 官方 API 有作废风险（停发免费 key、起诉爬虫）；连接器凭据以 `auth_kind` 抽象（`oauth` 现状 / `scrape` 未来代理爬虫），切换不改表。
...
hatch-radar/
├── apps/
│   ├── api/                    # 后端（NestJS，单进程）：HTTP /api + 鉴权 + 定时调度 + 内嵌任务执行（web 单独部署）
│   │   ├── prisma/             # Prisma schema + migrations（CLI 用；db:* 脚本归 api）
│   │   ├── prisma.config.ts    # Prisma 7 配置（连接串 / schema 路径；加载 apps/api/.env）
│   │   ├── src/
...
│   │   │   ├── common/ logger/              # DI 令牌·zod 管道·异常过滤器 / 日志
...
├── docs/                       # 设计与计划文档
├── docker-compose.yml          # 默认仅 db；profile full 起后端容器，profile tools 加 adminer
├── Dockerfile                  # 后端镜像（跑 TS 源；仅后端，不含 web）
├── pnpm-workspace.yaml
