...
端口 `HTTP_PORT`（默认 47878），同进程既发 `/api`、又同源托管 web SPA。鉴权恒开、fail-closed（网络位置不参与鉴权）：web 面向端点走会话 cookie + 能力闸（`SessionAuthGuard`），导出 / 同步走「设备签名 **或** 用户会话」双通道（`DeviceOrSessionGuard`），设备激活由一次性激活码自鉴权，`/api/health` 公开。
