...
> 顺序：数据层（表/枚举/哈希列/队列通用化）→ 翻译链路（translator + worker 分发）→ web 按钮 → 移动端导出携带译文。**设计稿，待实现。**

- [ ] Step 1 — Prisma：新增 `translations` + 三枚举；源表加哈希列；`analysis_jobs`→`jobs` + `job_type` + 改 `uniq_jobs_active_post` 为 `(post_id, job_type)`；迁移（含手工部分索引）。
- [ ] Step 2 — 入库补哈希：`upsertPosts` / `replaceComments` 在 decode 后计算 `title_hash`/`selftext_hash`/`body_hash`。
- [ ] Step 3 — translator：`packages/analysis/src/translator/*`（`translateWithClaudeAgent` + 纯函数 `translationFromMessage` + 结构化 schema + prompt）；本地语言检测短路。
- [ ] Step 4 — worker：`runJob` 按 `job_type` 分发 `runTranslationJob`（查未翻条目→批量→回写 `translations`）；`claimNextJob` 加 `job_type` 优先级（分析先）+ 运行期 `translationConcurrency` 上限（护栏 A+B）。
- [ ] Step 5 — API/设置：`/api/translations/posts/:id`（入队）+ "未翻译状态"查询端点；设置页加**翻译 provider/模型档**选择；成本聚合按 `job_type` 拆分。
- [ ] Step 6 — web：详情页翻译按钮（首次/增量/已翻状态机）+ 原文/中文切换。
- [ ] Step 7 — 导出：未翻确认框（直接导出 / 补全后导出）；payload 携带 `*_zh`；移动端中文优先渲染。
- [ ] Step 8 — 验收：typecheck/lint/单测/双进程冒烟（§十）。

> 偏差记录：实现过程中与本稿的出入在此追加。
