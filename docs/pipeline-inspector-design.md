...
1. `saveInsight` 是否已对 `post_id` 完全幂等（persist 节点重跑安全）？落地前复核。
2. `ai_call` 的多 Key 故障转移：首版走完整 failover（与生产一致）还是简化为「最优一把 + 失败重试节点」？倾向前者。
3. 是否需要把同一条帖子「用不同模型各跑一遍」做并排对比（A/B 检视）？可作为 M4+ 增强。
4. 实时更新长期是否升级为 SSE/WS（替代轮询）？取决于演示对延迟的要求。
